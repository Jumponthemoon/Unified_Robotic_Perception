...
def extract_state(hm_state,ct_inds):
    'hm_state 1,3,128,128'
    'inds 1,100'
    batch, cat, _ , _ = hm_state.size()
    state_inds = ct_inds.expand(ct_inds.size(0), cat, -1)
    state_scores = hm_state.view(batch, cat, -1)
    scores = state_scores.gather(2,state_inds)
    score, cls = torch.max(scores,1)
    return score,cls
...
def _topk(scores, K=40):
    ...
    #1,3,100     #1,3,100
    topk_scores, topk_inds = torch.topk(scores.view(batch, cat, -1), K)
    ###### topk_inds align with the index on corresponding heatmap ########
    topk_inds = topk_inds % (height * width)
    ########### x,y location on the map ###########
    topk_ys   = (topk_inds / width).int().float()
    topk_xs   = (topk_inds % width).int().float()
    ###### Extract overall high score point #######
    #1,100      #1,100  
...
def multi_pose_decode(
    heat, heat_state, wh, kps,dep,label_id,reg=None, hm_hp=None, hp_offset=None, K=100, kps_vis=None):
  batch, cat, height, width = heat.size()
  num_joints = kps.shape[1] // 2
  ######## Keep extremal point #######  
  heat = _nms(heat)
  ######## Extract topK point according to score value on heat map #########
  scores, inds, clses, ys, xs = _topk(heat, K=K)
  ######## Extract clses_state according to the index decoded from hm ########
  _, clses_state = extract_state(heat_state,inds)
  ######## Extract clses_state according to the index decoded from hm ########
  ######## Keypoint regression and visibility process ############
  kps = _transpose_and_gather_feat(kps, inds)
  kps_vis = _transpose_and_gather_feat(kps_vis,inds)
  kps = kps.view(batch, K, num_joints * 2)
  kps_vis = kps_vis.view(batch, K, num_joints)
  ...
  wh = wh.view(batch, K, 2)
  clses  = clses.view(batch, K, 1).float()
  clses_state = clses_state.view(batch, K, 1).float()
  scores = scores.view(batch, K, 1)
  ######## Formulate bbox ########
  bboxes = torch.cat([xs - wh[..., 0:1] / 2, 
                      ys - wh[..., 1:2] / 2,
  ...
  ######## Extract depth #########
  if dep is not None:
    depth = _transpose_and_gather_feat(dep, inds)
    depth = depth.view(batch, K, 1)
  else:
    depth = torch.ones((1,100,1)).to('cuda:0')
  ...
  if hm_hp is not None:
      ...
      kps_vis = kps_vis.view(batch, K, num_joints, 1).permute(
          0, 2, 1, 3).contiguous()
      reg_kps = kps.unsqueeze(3).expand(batch, num_joints, K, K, 2)
      hm_score, hm_inds, hm_ys, hm_xs = _topk_channel(hm_hp, K=K) # b x J x K
      ####### keypoint regress offset #######
      ...
      #### Define mask#######
      """
        If the predicted regress keypoint is out of bbox
        or too far away the point decode from the hm_hp,then 
        mask the point 
      """
      ...
      kps_vis = kps_vis.permute(0,2,1,3).contiguous().view(batch,K,num_joints)
      label_id = label_id.expand_as(clses)
  if kps_vis is not None:
    #  kps_vis = torch.sigmoid(_transpose_and_gather_feat(kps_vis, inds))
      detections = torch.cat([bboxes, scores, kps, kps_vis, clses, clses_state,label_id,depth], dim=2)
  else:
      detections = torch.cat([bboxes, scores, kps, clses, clses_state,label_id, depth], dim=2)
