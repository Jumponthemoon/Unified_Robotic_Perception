...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('task', default='perception',
    ...
    self.parser.add_argument('--demo', default='', 
                             help='path to image/ image folders/ video. '
                                  'or "webcam"')
    self.parser.add_argument('--save_folder', default='', 
    ...
    # model
    self.parser.add_argument('--arch', default='perception_34', 
                             help='model architecture. Currently tested'
                                  'res_18 | res_101 | resdcn_18 | resdcn_101 |'
                                  'dlav0_34 | dla_34 | hourglass')
    self.parser.add_argument('--vis_cam', action='store_true', 
    ...
    self.parser.add_argument('--trainval', action='store_true',
                             help='include validation in training and '
                                  'test on test set')
    self.parser.add_argument('--dynamic_weight', action='store_true',
                             help='use dynamic_weight .')
    self.parser.add_argument('--segmap', action='store_true',
                             help='seg heatmap.')
    self.parser.add_argument('--seg', action='store_true',
                             help='segmentation')
    self.parser.add_argument('--scene', action='store_true',
                             help='environment recognition')
    self.parser.add_argument('--dep', action='store_true',
                             help='flip data augmentation.')                             
    self.parser.add_argument('--vis_map', action='store_true',
                             help='flip data augmentation.')
    self.parser.add_argument('--ddd', action='store_true',
                             help='3d obj detection')
    self.parser.add_argument('--vis_val', action='store_true',
                             help='flip data augmentation.')
    # test
    ...
    self.parser.add_argument('--not_rand_crop', action='store_true',
                             help='not use the random crop data augmentation'
                                  'from CornerNet.')
    self.parser.add_argument('--sampler', action='store_true',
                             help='not use the random crop data augmentation'
                                  'from CornerNet.')
    ...
    self.parser.add_argument('--hm_state_weight', type=float, default=1,
                             help='loss state_weight for keypoint heatmaps.')
    ...
    self.parser.add_argument('--kp_vis_weight', type=float, default=1,
                             help='loss weight for human pose offset.')                             
    self.parser.add_argument('--hm_hp_weight', type=float, default=1,
                             help='loss weight for human keypoint heatmap.')
    self.parser.add_argument('--hm_state', action='store_false',
                             help='not use the random crop data augmentation'
                                  'from CornerNet.')                             
    # ddd
    ...
    self.parser.add_argument('--config', help="configuration file *.yml", type=str, required=False, default='config/config.yml')
    ...
    self.parser.add_argument('--eval_oracle_hm', action='store_true', 
                             help='use ground center heatmap.')
    self.parser.add_argument('--pck_thresholds', type=list, 
                             help='Pass pck thresholds needs to be evaluated.')                             
  ...
  def parse(self, args=''):
    ...
    opt.debug_dir = os.path.join(opt.save_dir)
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'exdet':
      ...
      if opt.reg_offset:
        opt.heads.update({'reg': 2})
    elif opt.task == 'perception':
      # assert opt.dataset in ['coco_hp']
      opt.flip_idx = dataset.flip_idx
      
      opt.heads = {'hm': 14, 'hm_state': 4, 'wh': 2, 'hps': 16, 'hps_vis':8}
      if opt.dep:
        opt.heads.update({'dep':1})
      if opt.segmap:
        opt.heads.update({'segmap':1})
      if opt.scene: 
        opt.heads.update({'scene':2})
      if opt.seg:
        opt.heads.update({'seg':19})
      if opt.reg_offset:
        opt.heads.update({'reg': 2})
      if opt.hm_hp:
